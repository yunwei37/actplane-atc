...
% 中：AI 智能体越来越多地在生产环境中运行，并通过 \emph{harness}——包裹 LLM 的软件运行时——来操作。
...
% 中：智能体 harness 的主要部分是策略引擎，它对智能体的行为执行安全性和有效性的策略。
Policies are often written in natural language or configuration files: our empirical study finds that 64\% of instruction-file statements are \emph{policies}, e.g., ``do not leak secrets'' or ``run tests before committing.''
% 中：策略通常以自然语言或配置文件编写：我们的实证研究发现，64\% 的指令文件语句是 \emph{策略}，例如"不要泄露密钥"或"提交前运行测试"。
Yet existing policy engines fail to connect policy intent to system actions, leaving a \emph{semantic gap}.
% 中：然而现有策略引擎无法将策略意图连接到系统行为，留下了 \emph{语义鸿沟}。
83\% of policies constrain system actions, where tool-call guardrails can miss indirect ones, and 81\% of projects contain cross-event policies that stateless interception cannot track.
% 中：83\% 的策略约束系统行为，其中工具调用护栏可能遗漏间接行为，而 81\% 的项目包含无状态拦截无法追踪的跨事件策略。
...
% 中：操作系统级沙箱式机制限制进程、文件和网络行为，但它们期望静态策略，而这对于 74\% 需要项目或任务上下文的规则难以预定义，且它们返回缺乏语义上下文的错误，进一步使智能体困惑。
...
% 中：我们的关键观察是，虽然安全约束是继承的，合规所需的上下文主要存在于最接近项目和任务的智能体处。
...
% 中：为弥合这一鸿沟，我们引入 \sys{}，一个运行时无关的策略引擎，它允许智能体在操作系统内核中声明和执行信息流控制（IFC）策略，并带有语义反馈。
Layered policy domains isolate agent policies and prevent weakening of inherited constraints.
% 中：分层策略域隔离智能体策略，并防止削弱继承的约束。
...
% 中：我们使用 eBPF 实现 \sys{}，并在来自实证研究的策略、编码任务基准和安全基准上评估它，显示了策略合规性的提升，包括在工具调用拦截无法观察的间接执行路径上，开销为 1.9\%--8.4\%。
