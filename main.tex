...
% 中：然而现有策略引擎无法将策略意图连接到系统级行为，留下了 \emph{语义鸿沟}。
83\% of policies constrain system-level events, where tool-call guardrails can miss indirect system actions, and 81\% of projects contain cross-event policies that stateless interception cannot track.
% 中：83\% 的策略约束系统级事件，其中工具调用护栏可能遗漏间接系统行为，而 81\% 的项目包含无状态拦截无法追踪的跨事件策略。
