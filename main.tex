...
83\% of policies constrain events that change the system state.
...
OS sandboxes expect static policy, hard to pre-define for 74\% of rules
needing context, and return errors that confuse the agent.
% 操作系统沙箱期望静态策略，而 74\% 的规则需要上下文难以预定义，且返回的错误令智能体困惑。
...
% 我们的关键观察是，虽然安全约束是继承的，合规所需的上下文主要存在于最接近项目和任务的智能体处。
...
81\% of projects contain cross-event policies that stateless interception cannot track, so we adopt labeled IFC for stateful propagation.
% 81\% 的项目包含跨事件策略，无状态拦截无法追踪，因此我们采用标记 IFC 进行有状态传播。
