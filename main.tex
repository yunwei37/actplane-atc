...
% AI 智能体越来越多地在生产环境中运行，并通过 \emph{harness}——包裹 LLM 的软件运行时——来操作。
...
% 智能体 harness 的主要部分是策略引擎，它对智能体的行为执行安全性和有效性的策略。
...
% 83\% 的策略约束系统行为，其中工具调用护栏可能遗漏间接行为，而 81\% 的项目包含无状态拦截无法追踪的跨事件策略。
...
% 操作系统级沙箱式机制限制进程、文件和网络行为，但它们期望静态策略，而这对于 74\% 需要项目或任务上下文的规则难以预定义，且它们返回缺乏语义上下文的错误，进一步使智能体困惑。
