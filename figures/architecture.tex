...
% Userspace.
\node[user] (feedback) at (-3.0,1.35) {Semantic\\feedback};
\node[dsl] (policy) at (0.0,1.35) {DSL\\policy};
...
\draw[arr] (engine.north) -- ++(0,0.45) -| node[pos=0.25,below,lbl] {violation} (feedback.south);
\draw[arr] (feedback.north) -- ++(0,0.45) -| (agent.south west);
...
\begin{scope}[on background layer]
  ...
  \node[fit=(feedback)(policy)(compiler), fill=blue!4, draw=blue!30,
        rounded corners=4pt, inner xsep=11pt, inner ysep=10pt,
