...
Development Process and Implementation Details are policy-heavy (86.7\% and 85.2\%, respectively), while Architecture is mostly descriptive (23.0\% policies) because directory layouts and design summaries dominate by line count.
% 开发流程和实现细节是策略密集的（分别为 86.7\% 和 85.2\%），而架构主要是描述性的（23.0\% 策略），因为目录布局和设计摘要在行数上占主导。
