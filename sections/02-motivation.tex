...
% \subsection{智能体 Harness 和策略}
\label{sec:background}

\textbf{AI agents and harnesses.}
% \textbf{AI 智能体和 harness。}
...
% AI 智能体将模型推理与外部工具、记忆和长时间运行的环境相结合，例如 Claude Code~\cite{anthropic2025claudecode}、Codex~\cite{openai2025codex}、Devin~\cite{devin2025} 和 OpenHands~\cite{wang2025openhands}。
...
% 当智能体追求其高级目标时，例如实现一个功能或预订一个航班，harness 需要对 \emph{系统行为}执行策略：在系统上发生的具体效果。
...
% 工具调用护栏和应用级 IFC 系统~\cite{wang2025agentspec,shi2025progent,invariant-guardrails,costa2025fides,debenedetti2025camel}在 harness 边界确定性地拦截。
...
% 这三个级别仅观察 harness 中介的请求，而不是工具开始执行后的系统级效果：一个间接子进程、一个 shell 调用或一个编译的二进制文件可以绕过工具边界。
...
% 快照（2026-05-23 UTC）包含 64 个仓库（中位数 20K GitHub 星，最小 9.8K，总计 2.2M）、84 个指令文件和 2,116 条提取的语句。
...
% 100 条语句的分层样本由人工标注者独立审查，可执行性标签另外由独立的 Claude 和 Codex 智能体交叉检查。
...
% 这个多数在行级别是不可见的：策略平均每条语句 3.6 行，而描述平均 6.8 行，因此同样的文件在按行测量时显得平衡（49\% 策略）。
...
% 架构主要是描述性的：目录布局和设计摘要使其按行数成为最大的主题之一，然而只有 23.0\% 的语句是策略。
...
% \subsection{E-RQ3：哪些策略需要操作系统级执行？}
...
% 出现四种反复模式：时间顺序（"提交前运行测试"）、跨文件一致性（"行为变化时更新文档"）、多步骤工作流（带有验证步骤的发布清单）和条件触发器（"如果你更改规范，也要更新 SDK"）。
All four require state that spans multiple operations: what ran, in what order, and what has changed since.
% 所有四种都需要跨越多个操作的状态：运行了什么，以什么顺序，以及从那时起发生了什么变化。
...
% 另外 64.2\% 需要项目上下文：策略提到"测试套件"、"代码检查器"或"上游源码"，其具体命令或路径需要从仓库中解析。
...
% 这加剧了执行挑战：需要跨事件 IFC 的策略很少指定编写规则所需的具体命令和路径。
