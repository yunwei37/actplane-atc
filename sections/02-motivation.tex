...
We use \emph{runtime policy enforcement system} for the part of an AI agent harness that enforces safety, security, and compliance policies while the agent executes. Existing enforcement operates at three levels, each with limitations.
% 我们使用 \emph{运行时策略执行系统}指代 AI 智能体 harness 中在智能体执行时执行安全、安全性和合规策略的部分。现有执行在三个级别运行，每个都有局限性。
At the first level, prompt instructions rely on the model's own compliance capabilities~\cite{liu2024lost,jiang2024followbench,qi2025agentif} but are vulnerable to prompt injection~\cite{greshake2023indirectprompt,zhan2024injecagent,debenedetti2024agentdojo}.
% 第一级，提示词指令依赖于模型自身的遵循能力~\cite{liu2024lost,jiang2024followbench,qi2025agentif}，但容易受到提示词注入攻击~\cite{greshake2023indirectprompt,zhan2024injecagent,debenedetti2024agentdojo}。
At the second level, separate agents or LLM guards can check prompts, responses, or action trajectories at runtime~\cite{rebedea2023nemo,xiang2025guardagent,chennabasappa2025llamafirewall}, but these are inherently probabilistic.
% 第二级，独立的智能体或 LLM 守卫可以在运行时检查提示词、响应或行为轨迹~\cite{rebedea2023nemo,xiang2025guardagent,chennabasappa2025llamafirewall}，但本质上是概率性的。
At the third level, tool-call guardrails and application-level IFC systems~\cite{wang2025agentspec,shi2025progent,invariant-guardrails,costa2025fides,debenedetti2025camel} intercept at the harness boundary deterministically, but all three levels observe only harness-mediated requests, not the system-level effects once a tool starts executing, so an indirect subprocess, a shell-out, or a compiled binary can bypass the tool boundary.
...
Each policy is classified into the first matching tier of an enforcement waterfall (Figure~\ref{fig:waterfall-enforcement}): \emph{semantic-only} covers reasoning, communication, or output style; \emph{content} covers predicates over file contents; \emph{per-event} covers a single command, file access, or network connection; and \emph{cross-event} covers history, ordering, lineage, or information flow.
% 每条策略被分类到执行瀑布的第一个匹配层级（图~\ref{fig:waterfall-enforcement}）：\emph{仅语义}涵盖推理、沟通或输出风格；\emph{内容}涵盖文件内容上的谓词；\emph{单事件}涵盖单个命令、文件访问或网络连接；\emph{跨事件}涵盖历史、顺序、血统或信息流。
...
Four recurring patterns emerge. Temporal ordering requires sequencing constraints such as ``run tests before committing.'' Cross-file consistency links changes across artifacts, for example ``update docs when behavior changes.'' Multi-step workflows enforce release checklists with verification gates. Conditional triggers couple operations, as in ``if you change specs, also update SDK.'' All four patterns require state that spans multiple operations because the engine must track what ran, in what order, and what has changed since, and most of these operations involve files, processes, and network connections.
% 出现四种反复模式。时间顺序要求排序约束如"提交前运行测试"。跨文件一致性链接跨工件的变更，例如"行为变化时更新文档"。多步骤工作流执行带有验证门的发布清单。条件触发器耦合操作，如"如果你更改规范，也要更新 SDK"。所有四种模式都需要跨越多个操作的状态，因为引擎必须追踪运行了什么、以什么顺序、以及从那时起发生了什么变化，而且大多数操作涉及文件、进程和网络连接。
