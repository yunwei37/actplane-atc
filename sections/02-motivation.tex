...
\noindent\textbf{Instruction files are mainly policies: the median repository has 71\% policies.}
% 中：\noindent\textbf{指令文件主要是策略：中位数仓库有 71\% 策略。}
...
% 中：然而许多策略约束跨越多个操作的系统行为，这对工具调用护栏是不可见的（E-RQ3），因此规则必须足够具体以进行确定性操作系统级跨事件执行。
