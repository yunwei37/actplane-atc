...
% 中：\texttt{IDENT}命名标签或规则，\texttt{PATTERN}、\texttt{ARG} 和 \texttt{STRING}是带引号的字符串。
...
% 中：当操作目标匹配、\texttt{if}表达式在主体标签上成立、且可选的 \texttt{unless}条件为假时，子句匹配。
...
% 中：普通的 \texttt{after exec "pytest"}是锁存的：一旦门控在进程血统中发生，它就保持满足。
Adding \texttt{since write "src/**"} makes the gate stale after a later matching write.
% 中：添加 \texttt{since write "src/**"}使门控在后续匹配写入后过期。
...
% 中：\texttt{exits N}限定符仅对 \texttt{after exec}门控有效，在匹配进程以状态 \texttt{N}退出后打开门控。
The optional \texttt{ARG} in \texttt{exec} patterns is a single argv-token predicate, for example \texttt{exec "git" "commit"}.
% 中：\texttt{exec}模式中的可选 \texttt{ARG}是单个 argv-token 谓词，例如 \texttt{exec "git" "commit"}。
...
% 中：RQ2 轨迹判断器接收真实标签、原始轨迹记录、任何装置文件（用于间接执行轨迹）和观察到的运行器结果。
It outputs a single confusion-matrix label (\textsc{TP}, \textsc{TN}, \textsc{FP}, \textsc{FN}, or \textsc{unclear}) with a rationale.
% 中：它输出单个混淆矩阵标签（\textsc{TP}、\textsc{TN}、\textsc{FP}、\textsc{FN}或 \textsc{unclear}）及其理由。
