...
% 中：\texttt{exits N} 限定符仅对 \texttt{after exec} 门控有效，并在一个匹配的进程以状态 \texttt{N} 退出后打开门控。
The optional \texttt{ARG} in \texttt{exec} patterns is a single argv-token predicate, for example \texttt{exec "git" "commit"}.
% 中：\texttt{exec} 模式中的可选 \texttt{ARG} 是一个单 argv-token 谓词，例如 \texttt{exec "git" "commit"}。
Extending \texttt{op\_pattern} and temporal gates is future work.
% 中：扩展 \texttt{op\_pattern} 和时间门控是未来工作。

\section{LLM-as-Judge Prompt}
% 中：\section{LLM 作为判断器提示词}
...
% 中：RQ2 轨迹判断器接收一个真实标签、原始轨迹记录、任何装置文件（用于间接执行轨迹）和观察到的运行器结果。
It outputs a single confusion-matrix label (\textsc{TP}, \textsc{TN}, \textsc{FP}, \textsc{FN}, or \textsc{unclear}) with a rationale.
% 中：它输出一个单一的混淆矩阵标签（\textsc{TP}、\textsc{TN}、\textsc{FP}、\textsc{FN} 或 \textsc{unclear}）及其理由。
