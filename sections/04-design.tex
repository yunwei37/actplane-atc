...
\textbf{C1: Policy specification at the right granularity}
(\S\ref{sec:dsl}).
...
main.'' Kernel enforcement operates over a much lower-level vocabulary:
...
ambiguous to enforce deterministically; one written directly at the OS
event level is verbose, brittle, and exposes kernel machinery that agents
and humans cannot reliably inspect. The challenge is to define a compact
policy vocabulary at the right granularity: close enough to system events
to compile, but high enough to preserve the directive's context, scope,
and rationale.

\textbf{C2: Stateful enforcement at syscall speed} (\S\ref{sec:ifc}).
...
the full behavior, but enforcement cannot afford to reconstruct or query
complete execution traces on every syscall. The challenge is to maintain
enough policy-relevant history across tools, subprocesses, files, and
network endpoints for deterministic checks without imposing prohibitive
...
narrower scope, or the current task may need a pre-authorized exception
path through a standing rule. Letting the constrained actor write or
specialize policy creates a self-modification problem: the agent must be
...
The DSL provides the intermediate specification granularity between AI
agent intent and enforceable system actions.
...
may need a pre-authorized exception path through a standing rule; but
