...
The empirical study identifies 607~OS-enforceable policies.
% 该实证研究识别了 607 条操作系统可执行策略。
RQ1 uses all of them.
% RQ1 使用所有这些策略。
A Codex agent~\cite{openai2025codex} with GPT-5.5~\cite{openai2026gpt55} reads each policy, the repository context, and the DSL reference, then writes the corresponding \sys{} rule.
% 一个 Codex 智能体~\cite{openai2025codex}配合 GPT-5.5~\cite{openai2026gpt55}读取每条策略、仓库上下文和 DSL 参考，然后编写相应的 \sys{} 规则。
The runner invokes the \sys{} compiler and retries once on failure.
% 运行器调用 \sys{} 编译器，失败时重试一次。
The translator produced rules for all 607~policies, and all of them compiled.
% 该翻译器为所有 607 条策略生成了规则，且它们全部编译通过。
Only 2~policies required a retry, giving a 0.3\% retry rate and no compilation failures.
% 只有 2 条策略需要重试，给出 0.3\% 的重试率且无编译失败。
...
% 生成实验共使用了 1.7M 非缓存输入 token 和 177k 输出 token，每条策略 \$0.028。
At typical US software-engineer rates, manually writing ten rules per hour would cost roughly \$11 per rule.
% 按典型美国软件工程师费率，每小时手动编写十条规则大约每条规则 \$11。
Full translation takes about 34\,min using 7 subagents, 4 running in parallel.
% 使用 7 个子智能体完整翻译约需 34 分钟，4 个并行运行。
The 607~policies expand to 1{,}283~rule lines.
% 607 条策略扩展为 1,283 条规则行。

\noindent\textbf{Most policies are structurally simple.}
% \noindent\textbf{大多数策略在结构上是简单的。}
217 (36\%) have a single enforcement clause, 231 (38\%) have two, and 101 (17\%) have three.
% 217 条（36\%）有一个单一的执行子句，231 条（38\%）有两个，101 条（17\%）有三个。
Policies average 71 tokens under the OpenAI \texttt{o200k\_base} tokenizer: per-event policies average 62 tokens, cross-event 86, with p95 at 122 overall.
% 策略平均 71 个 token（单事件：62，跨事件：86；p95：总体 122，单事件 94，跨事件 152），使用 OpenAI \texttt{o200k\_base} 分词器。

\noindent\textbf{Most DSL features are exercised.}
% \noindent\textbf{大多数 DSL 特性被运用。}
...
% 按效果，66\% 的子句是通知，29\% 阻止，5\% 终止；按钩子，60\% 针对 exec，37\% 写入，unlink（0.9\%）、connect（0.8\%）和 open（0.5\%）。
...
% 28\% 的策略使用 \texttt{after/since} 时间门控；214 条使用 \texttt{unless}；82 条在标签表达式中使用 \texttt{or}；13 条使用 \texttt{lineage-includes}；9 条使用 \texttt{target}。
95 policies (15.7\%) define file sources for information-flow tracking beyond agent provenance.
% 95 条策略（15.7\%）定义文件源以进行超越智能体来源的信息流追踪。
...
% \texttt{declassify}/\texttt{endorse} 变换在此语料库中未使用，因为它们设计用于智能体定义清除累积标签的批准路径，缓解更长会话中标签单调性导致的过度标记。
...
% \paragraph{基准动机。}
To measure the effectiveness of \sys{} compared to other tool-level checkers and guardrails under the same agent trajectory, we build a new benchmark using similar methodology to existing ones, but designed to challenge runtime enforcement systems with indirect execution paths and cross-event policies.
% 为衡量 \sys{} 与其他工具级检查器和护栏在相同智能体轨迹下的有效性，我们使用类似于现有基准的方法构建新基准，但设计为通过间接执行路径和跨事件系统行为挑战运行时执行系统。
...
% 现有执行基准如 ShieldAgent-Bench~\cite{li2025shieldagent} 和 ToolSafe~\cite{chen2026toolsafe}在工具调用或轨迹级别评估护栏精度和召回。
...
% AgentSpec、Progent、FIDES 和 CaMeL~\cite{wang2025agentspec,shi2025progent,costa2025fides,debenedetti2025camel}中的运行时执行评估在 AgentDojo 场景~\cite{debenedetti2024agentdojo}上衡量攻击成功率降低。
...
% \paragraph{规则集和范围。}
RQ2 samples from the 607~OS-enforceable policies.
% RQ2 从 607 条操作系统可执行策略中采样。
Semantic-only and content policies are excluded: the former have no system-observable effect, and the latter require linter rather than OS-level IFC.
% 仅语义的和内容策略被排除：前者没有系统可观察的效果，后者需要代码检查器而非操作系统级 IFC。
...
% 我们评估 38 条随机选择的规则，从 15 个仓库各采样 2--3 条，以覆盖单事件和跨事件规则、项目和任务上下文要求以及常见仓库策略主题。
The final manifest contains 20 per-event and 18 cross-event rules.
% 最终清单包含 20 条单事件和 18 条跨事件规则。
...
% 一个 Codex CLI 智能体~\cite{openai2025codex}配合 GPT-5.5~\cite{openai2026gpt55}读取规则和仓库后生成五条轨迹。
The policy DSL and all enforcement systems, including \sys{}, are invisible to it to avoid information leakage.
% 策略 DSL 和所有执行系统（包括 \sys{}）对其不可见，以避免信息泄露。
...
% 三个违规族反映了部署的编码智能体中观察到的执行路径类别~\cite{anthropic2026containment,maloyan2026sok}：直接违规轨迹通过工具调用触发禁止操作；脚本轨迹将脚本编写和执行拆分到单独的工具调用中，将副作用移入未监控的子进程~\cite{maloyan2026sok}；隐藏轨迹将副作用嵌入辅助工件中，其入口点看起来无害~\cite{wu2026skillject}，因此真实操作仅在操作系统级别可观察。
...
% 我们在 codex-sdk 上重放它们以触发执行系统，然后测试的智能体继续最多五个真实工具步骤，衡量智能体在收到语义反馈后采取的下一个行动。
...
% RQ2 在相同智能体轨迹上比较五个策略执行系统（图~\ref{fig:rq2-pipeline}）：基于提示的行动过滤器、工具输入正则策略、FIDES~\cite{costa2025fides}（工具级 IFC）、无反馈内核 IFC（\textsc{\sys{}-opaque}）和完整 \sys{}。
...
% 提示、正则和 FIDES 基线实例化了常见的运行时执行层：对提议行动的 LLM/提示基础护栏~\cite{rebedea2023nemo,xiang2025guardagent}；Claude Code hooks~\cite{anthropic2026hooks}、Codex 批准策略~\cite{openai2025codex}和 AgentSpec~\cite{wang2025agentspec}中部署的确定性工具调用策略；以及 FIDES 工具级 IFC 策略。
...
% 作为沙箱式内核 IFC 引擎~\cite{pasquier2018camquery,flume,zeldovich2006histar}的受控代理和语义反馈消融，我们使用无反馈的 \sys{} 引擎作为无反馈内核 IFC 基线。
...
% 另一个翻译智能体（Claude Code~\cite{anthropic2025claudecode}配合 Claude Opus~4.6）读取规则文本和克隆的仓库，然后为每个非提示系统生成相应的策略工件：工具正则的正则表达式、FIDES 策略和 \sys{} 的 DSL 规则，不访问真实轨迹以避免数据泄露。
...
% 在每个设置中，判断器看到真实标签、策略执行系统是否触发及其反馈、以及智能体在后续步骤中做了什么。
...
% 类似于其他基准~\cite{zheng2023judging,debenedetti2024agentdojo,li2025shieldagent,chen2026toolsafe}，我们使用 LLM 作为判断器评估~\cite{zheng2023judging}。
...
% TP 表示违规轨迹通过执行干预或智能体意识恢复到策略合规；TN 表示合规轨迹在无错误干预下完成；FP 表示合规轨迹被错误阻止或引导；FN 表示违规轨迹被遗漏。
...
% 我们报告 \emph{决策合规率}（DCR），即最终结果策略正确的轨迹条件决策点的比例：\(\mathrm{DCR}=(\mathrm{TP}+\mathrm{TN})/(\mathrm{TP}+\mathrm{TN}+\mathrm{FP}+\mathrm{FN})\)。
DCR is end-to-end from natural language policy to agent behavior: it includes policy translation, runtime enforcement, feedback delivery, and agent recovery.
% DCR 是从自然语言策略到智能体行为的端到端：它包括策略翻译、运行时执行、反馈传递和智能体恢复。
...
% 差距在脚本和隐藏轨迹族上扩大，其中提示过滤、工具正则和 FIDES 急剧下降，三者在隐藏轨迹上为零或接近零，因为禁止效果发生在子进程内或中性入口点后面。
...
% \sys{} 通过内核级标签传播保持检测，即使智能体间接调用也能跟踪监控效果。
...
% 跨事件规则（38 条中的 18 条）进一步扩大差距：工具调用基线缺乏跟踪顺序约束（如"提交前运行测试"）所需的持久状态。
...
% 翻译质量决定两者：规则过窄会遗漏违规（降低检测），而规则过宽会在 18 个误报案例中的 17 个匹配合规操作（提高误报）。
...
% 在 \textsc{\sys{}-opaque} 下，执行相同内核规则，同样的 17 条轨迹被判断为 TN，因为智能体缺乏反馈通道并通过替代路径完成任务。
...
% 我们将每条假阴性轨迹的证据和纠正反馈提供给翻译智能体，让其修订 DSL 规则一次。
Rerunning the 28~FN traces with revised rules recovers 26 (92.9\%).
% 用修订后的规则重新运行 28 条 FN 轨迹，恢复 26 条（92.9\%）。
...
The \textsc{\sys{}-opaque} ablation removes semantic feedback and reduces correct responses from 144 to 102: FN rises from 28 to 87 while FP drops from 18 to 1.
It detects 75.4\% of violations, but only 31.4\% of those detections lead to compliance.
% \textsc{\sys{}-opaque} 消融移除语义反馈，正确响应从 144 降至 102（FN 从 28 升至 87；FP 从 18 降至 1）：它检测到 75.4\% 的违规，但只有 31.4\% 的检测导致合规。
...
% 反馈还间接提高检测：收到第一次拒绝后纠正载荷的智能体会修改其方法，当修改后的尝试仍然违规时引擎会捕获它。
...
% DeepSeek-Pro~V4~\cite{deepseek2026v4}端到端复现保持系统排名（图~\ref{fig:rq2-dcr}）：\sys{} 以 77.4\% DCR 保持最高。
Per-cell agreement between the two model settings yields Cohen's $\kappa=0.822$.
Deterministic enforcement shows the highest stability: tool-regex $\kappa=0.920$, FIDES $\kappa=0.919$, \sys{} $\kappa=0.852$.
Prompt-filter is lowest at $\kappa=0.592$, consistent with the expected gradient from objective to subjective evaluation.
% 两个模型设置之间的每单元一致性产生 Cohen's $\kappa=0.822$，确定性执行稳定性最高（工具正则 $\kappa=0.920$、FIDES $\kappa=0.919$、\sys{} $\kappa=0.852$），提示过滤最低（$\kappa=0.592$），与从客观到主观评估的预期梯度一致。
...
We measure end-to-end overhead on two workloads under no-hit \sys{} configurations, where policies are loaded but no rule fires during the workload.
AP-$N$ denotes \sys{} with $N$ active rules.
The first workload is an agent trace suite that replays 68~tool actions with 20~Bash subprocesses.
The second is a Linux kernel build: \texttt{defconfig} plus \texttt{vmlinux}, \texttt{make -j24}, clean output directory.
% 我们在无命中 \sys{} 配置下测量两个工作负载的端到端开销，即策略已加载但工作负载期间无规则触发（AP-$N$ 表示有 $N$ 条活动规则的 \sys{}）：（1）智能体轨迹套件重放智能体轨迹（68 个工具操作，20 个 Bash 子进程），（2）Linux 内核构建（\texttt{defconfig} + \texttt{vmlinux}，\texttt{make -j24}，干净输出目录）。
...
% 固定工作负载消除 LLM 推理方差并隔离 \sys{} 的运行时成本。
...
\paragraph{Goal.}
% \paragraph{目标。}
Unlike RQ2, which isolates single decision points, RQ4 evaluates \sys{} on complete coding-agent tasks in real repositories, scored by the official OctoBench checklist judge.
Tasks include implementing features, fixing bugs, and configuring builds.
...
% 改进集中在任务特定检查上：用户查询奖励比基线提高 9.9 分，实现/测试奖励提高 9.7 分，合规奖励显示较小的 2.8 分提升。
