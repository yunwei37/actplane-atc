...
All experiments run on a machine with an Intel Core Ultra~9~285K CPU with 24~hardware cores, 125\,GiB RAM, and Linux~6.15.11.
...
To validate correctness, we apply the same methodology as the empirical study (\S\ref{sec:empirical}), where two independent LLM agents cross-check all policies and human annotators review 100 randomly selected ones.
% 为验证正确性，我们采用与实证研究（\S\ref{sec:empirical}）相同的方法论：两个独立的 LLM 智能体交叉检查所有策略，人工标注者审查 100 条随机选择的策略。

Agent-based translation is orders of magnitude cheaper than manual authoring. The full 607-policy corpus cost \$0.028 per policy using 1.7M input tokens and 177k output, compared with roughly \$11 per rule at typical US software-engineer rates.
% 基于智能体的翻译比人工编写便宜几个数量级。全部 607 条策略语料库每条成本 \$0.028，使用 1.7M 输入 token 和 177k 输出，而按典型美国软件工程师费率约每条规则 \$11。
The translation completed in 34 minutes using 7 subagents with 4 running in parallel, expanding 607 policies into 1{,}283 rule lines.
...
74\% have at most two enforcement clauses, and even the 95th percentile stays under 152 tokens with mean 71 using the OpenAI \texttt{o200k\_base} tokenizer.
% 74\% 最多有两个执行子句，即使第 95 百分位也在 152 token 以下，均值 71，使用 OpenAI \texttt{o200k\_base} 分词器。
Cross-event policies are slightly longer at mean 86 tokens than per-event ones at 62, but both remain well within single-prompt context limits.
% 跨事件策略略长，均值 86 token，而单事件策略为 62，但两者都在单提示上下文限制范围内。
...
Hooks concentrate on code execution at 60\% exec and file mutation at 37\% write, with network and cleanup operations under 1\% each.
% 钩子集中在代码执行 60\% exec 和文件变更 37\% write，网络和清理操作各占不到 1\%。

Cross-event features see substantial use, with 28\% of policies using an \texttt{after/since} temporal gate and 214 using \texttt{unless} to encode exceptions, while the 95 policies that define file sources demonstrate information-flow tracking beyond simple agent provenance.
% 跨事件特性使用广泛，28\% 的策略使用 \texttt{after/since} 时间门控，214 条使用 \texttt{unless} 编码例外，而定义文件源的 95 条策略展示了超越简单智能体来源的信息流追踪。
Only \texttt{declassify}/\texttt{endorse} transforms are unused, as expected, because they address over-tainting in long-running sessions, a scenario absent from this task-oriented corpus.
% 仅 \texttt{declassify}/\texttt{endorse} 变换未使用，这在预期内，因为它们解决长会话中的过度标记问题，而这种场景在面向任务的语料库中不存在。
...
% \paragraph{规则集和范围。}
RQ2 samples 38 rules from the 607 OS-enforceable policies, excluding semantic-only policies with no system-observable effect and content policies that require linters rather than OS-level IFC.
% RQ2 从 607 条操作系统可执行策略中采样 38 条规则，排除无系统可观察效果的仅语义策略和需要代码检查器而非操作系统级 IFC 的内容策略。
...
This design yields 190 traces from 38 rules with 5 traces each.
% 此设计从 38 条规则各 5 条轨迹产生 190 条轨迹。
...
We manually reviewed the samples the judge flagged for double-checking, plus 50 random judgments. All reviewed cases matched our assessment.
% 我们手动审查了判断器标记为需要复查的样本以及额外 50 个随机判断。所有审查案例均与我们的评估一致。
...
\caption{RQ2 breakdown by trace family. Cells show DCR in percent for each system$\times$family; darker is higher.}
% \caption{按轨迹族的 RQ2 分解。单元格显示每个系统$\times$族的 DCR 百分比；颜色越深越高。}
...
Translation quality drives both rates, since rules too narrow miss violations while rules too broad match compliant actions, causing 17 of the 18 false positives.
% 翻译质量驱动这两个比率，因为规则过窄会遗漏违规，而规则过宽会匹配合规操作，导致 18 个误报中的 17 个。
...
The DeepSeek-Pro~V4~\cite{deepseek2026v4} end-to-end replication preserves the system ranking, with \sys{} highest at 77.4\% DCR and per-cell agreement between the two model settings yielding Cohen's $\kappa=0.822$.
% DeepSeek-Pro~V4~\cite{deepseek2026v4}端到端复现保持系统排名，\sys{} 以 77.4\% DCR 最高，两个模型设置之间的每单元一致性产生 Cohen's $\kappa=0.822$。
% 两个模型设置之间的每单元一致性产生 Cohen's $\kappa=0.822$。
As expected, deterministic systems show the highest stability with tool-regex at $\kappa=0.920$, FIDES at $\kappa=0.919$, and \sys{} at $\kappa=0.852$, while prompt-filter is lowest at $\kappa=0.592$ because its decisions depend on model-specific reasoning.
...
We measure end-to-end overhead on two workloads under no-hit \sys{} configurations where policies are loaded but no rule fires. AP-$N$ denotes \sys{} with $N$ active rules.
% 我们在无命中 \sys{} 配置下测量两个工作负载的端到端开销，策略已加载但无规则触发。AP-$N$ 表示有 $N$ 条活动规则的 \sys{}。
The first workload is an agent trace suite that replays 68 tool actions with 20 Bash subprocesses. The second is a Linux kernel build using \texttt{defconfig} plus \texttt{vmlinux} with \texttt{make -j24} on a clean output directory.
% 第一个工作负载是重放 68 个工具操作和 20 个 Bash 子进程的智能体轨迹套件。第二个是 Linux 内核构建，使用 \texttt{defconfig} + \texttt{vmlinux}，\texttt{make -j24}，干净输出目录。
...
We measure \sys{}'s per-event latency for five syscall types, \texttt{fork}, \texttt{exec}, \texttt{open}, \texttt{write}, and \texttt{connect}, under five no-hit configurations: native, and \sys{} with 1, 10, 32, and 100 active rules.
...
\texttt{fork} and \texttt{exec} incur the highest absolute added cost at 3--69\,\textmu s under AP-100 but remain modest relative to their native latency because process-management and scheduler cost dominates the baseline.
...
The cumulative \sys{} overhead of an entire tool-call's syscall sequence is 5--6 orders of magnitude smaller than a single LLM inference turn of 2--10\,s. All overhead measurements use no-hit configurations.
% 整个工具调用系统调用序列的累积 \sys{} 开销比单次 LLM 推理轮 2--10\,s 小 5--6 个数量级。所有开销测量使用无命中配置。
...
% \paragraph{基准和子集。}
OctoBench~\cite{octobench} is a coding benchmark with 217 tasks spanning system prompts, tool schemas, project files (\texttt{CLAUDE.md}, \texttt{AGENTS.md}), and user queries. We use its official evaluator unchanged.
% OctoBench~\cite{octobench}是一个编码基准，包含 217 个任务，涵盖系统提示、工具模式、项目文件（\texttt{CLAUDE.md}、\texttt{AGENTS.md}）和用户查询。我们原样使用其官方评估器。
% 我们原样使用官方评估器。
Because \sys{} cannot observe purely semantic checks such as tone, we select a 21-task subset with 61 rules whose user-query checklist contains at least one targeted OS-enforceable item covering command, file operation, or tool-execution.
% 因为 \sys{} 无法观察纯语义检查如语气，我们选择一个 21 任务子集，61 条规则，其用户查询清单包含至少一个有针对性的操作系统可执行项，涵盖命令、文件操作或工具执行。
...
Each task runs under three conditions: baseline with no enforcement, Claude Code hooks~\cite{anthropic2026hooks} using the official sandbox approval policy, and \sys{} with OS-level hooks plus semantic feedback.
% 每个任务在三个条件下运行：基线无执行、Claude Code hooks~\cite{anthropic2026hooks}使用官方沙箱批准策略、以及 \sys{} 操作系统级钩子加语义反馈。
...
\paragraph{Metrics.}
% \paragraph{指标。}
The primary metric is official OctoBench reward, and we additionally report three diagnostic submetrics from the same checklist results, namely \emph{user-query reward} for user-task checks only, \emph{implementation/test reward} for implementation, testing, configuration, and modification checks, and \emph{compliance reward} for compliance-typed checks; all submetrics use the official LLM-based checklist judge.
% 主要指标是官方 OctoBench 奖励，我们另外报告来自相同清单结果的三个诊断子指标，即 \emph{用户查询奖励}（仅用户任务检查）、\emph{实现/测试奖励}（实现、测试、配置和修改检查）和 \emph{合规奖励}（合规类型检查）；所有子指标使用官方 LLM 基础清单判断器。
...
Each task runs inside a Docker-isolated OpenHands~\cite{wang2025openhands} runtime with service dependencies like GitLab and ownCloud plus an independent evaluator that scores the final workspace state.
% 每个任务在 Docker 隔离的 OpenHands~\cite{wang2025openhands}运行时中运行，带有 GitLab、ownCloud 等服务依赖和对最终工作空间状态评分的独立评估器。
We run every task under two conditions: \emph{baseline} with OpenHands and no enforcement, and \emph{\sys{}} with OpenHands plus an OS-level \sys{} watcher loading a per-task policy.
% 我们在两个条件下运行每个任务：\emph{基线}为无执行的 OpenHands，\emph{\sys{}}为 OpenHands 加操作系统级 \sys{} 监视器加载每任务策略。
This setup differs from RQ1--RQ4 in two dimensions: the agent framework uses OpenHands rather than Codex CLI, and the workload covers workplace safety tasks rather than repository coding tasks.
% 此设置在两个维度上与 RQ1--RQ4 不同：智能体框架使用 OpenHands 而非 Codex CLI，工作负载涵盖工作场所安全任务而非仓库编码任务。
...
Our policy generator mirrors this by having a Codex agent with GPT-5.5~\cite{openai2026gpt55} read only the task description and extract explicit file paths, system commands, and high-confidence sink categories such as credentials, cron jobs, SSH keys, account mutations, and network egress, producing a per-task \sys{} policy compiled to an eBPF configuration blob.
% 我们的策略生成器反映这一点：一个 Codex 智能体配合 GPT-5.5~\cite{openai2026gpt55}仅读取任务描述，提取显式文件路径、系统命令和高置信度汇类别如凭证、cron 作业、SSH 密钥、账户变更、网络出口，生成编译为 eBPF 配置 blob 的每任务 \sys{} 策略。
...
\caption{RQ5: Outcome distribution on all 361 OpenAgentSafety tasks. \sys{} reduces baseline-unsafe outcomes from 106 to 28 at 74\% prevention rate using policies generated from task descriptions without human tuning.}
...
These are hook-coverage gaps, not policy errors, because the generator produced a matching rule for the intended sink but the agent reached the same effect through a path the current hook set does not observe.
% 这些是钩子覆盖差距而非策略错误，因为生成器为预期汇生成了匹配规则，但智能体通过当前钩子集未观察的路径达到相同效果。
Chat and content harms are outside \sys{}'s scope by design, while service-side artifacts require protocol-level hook extensions.
...
% \textbf{内核内来源和信息流。}
Provenance systems such as PASS, Hi-Fi, LPM, and CamFlow~\cite{muniswamy2006pass,pohly2012hifi,bates2015lpm,pasquier2017camflow} record audit-grade graphs but do not compile agent-oriented policy or return semantic feedback.
% 来源系统如 PASS、Hi-Fi、LPM 和 CamFlow~\cite{muniswamy2006pass,pohly2012hifi,bates2015lpm,pasquier2017camflow}记录审计级图，但不编译面向智能体的策略或返回语义反馈。
CamQuery~\cite{pasquier2018camquery} is the closest prior system in that it propagates labels and can deny matching operations, but it targets adversarial intrusions with general provenance queries rather than an agent-facing DSL with semantic feedback.
