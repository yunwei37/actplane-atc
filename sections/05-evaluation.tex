...
% 中：\item[RQ1（DSL 覆盖）] 一个翻译智能体能否为所有 607 条操作系统可执行策略生成 \sys{} 规则？
...
% 中：\item[RQ2（策略引擎有效性）] 与基线相比，\sys{} 是否提高了来自我们实证研究的规则在直接和间接执行路径上的策略合规性？
...
% 中：\item[RQ4（真实世界编码任务）] 在真实仓库中的编码智能体任务上，\sys{} 是否提高了策略合规性和任务成功率？
...
% 中：\item[RQ5（编码之外的安全性）] 在涵盖数据处理、系统管理和工作场所任务的安全基准上，\sys{} 的智能体生成策略，作为更高权限域加载，是否能阻止不安全的智能体行为？
\end{description}
...
% 中：一个 Codex 智能体~\cite{openai2025codex}配合 GPT-5.5~\cite{openai2026gpt55}读取每条策略、仓库上下文和 DSL 参考，然后编写相应的 \sys{} 规则。
...
% 中：为验证正确性，我们采用与实证研究（\S\ref{sec:empirical}）相同的方法论：两个独立的 LLM 智能体交叉检查所有策略，人工标注者审查 100 条随机选择的策略。
...
% 中：217 条（36\%）有一个单一的执行子句，231 条（38\%）有两个，101 条（17\%）有三个。
...
% 中：仅语义的和内容策略被排除：前者没有系统可观察的效果，后者需要代码检查器而非操作系统级 IFC。
...
% 中：DCR 是从自然语言策略到智能体行为的端到端：它包括策略翻译、运行时执行、反馈传递和智能体恢复。
...
% 中：\Description{分组条形图比较两个模型设置下的总体策略合规率：Qwen3.6-27B、DeepSeek-Pro V4}
\label{fig:rq2-dcr}
\end{figure}
...
% 中：差距在脚本和隐藏轨迹族上扩大，其中提示过滤、工具正则和 FIDES 急剧下降，三者在隐藏轨迹上为零或接近零，因为禁止效果发生在子进程内或中性入口点后面。
