...
Effects skew toward observation: 66\% of clauses are notify, 29\% block, and only 5\% kill, reflecting that most policies monitor rather than prevent.
...
Each trace simulates realistic coding-agent work: user prompts, file I/O, shell commands, and build/test invocations on a real repository.
% 每条轨迹模拟真实的编码智能体工作：用户提示、文件 I/O、shell 命令以及在真实仓库上的构建/测试调用。

For each rule, the agent produces two compliant traces and three violation traces.
% 对于每条规则，智能体生成两条合规轨迹和三条违规轨迹。
The compliant traces stress false-positive detection: allowed-effect traces exercise permitted actions close to the boundary, and lookalike traces perform technically compliant operations that resemble violations.
...
DCR is end-to-end: it captures policy translation, runtime enforcement, feedback delivery, and agent recovery.
% DCR 是端到端的：它捕获策略翻译、运行时执行、反馈传递和智能体恢复。
...
The gap concentrates on violation traces: \sys{} correctly resolves 86 of 114 (75.4\%), compared with 27--44 for baselines (2.0--3.2$\times$).
% 差距集中在违规轨迹上：\sys{} 正确解决 114 条中的 86 条（75.4\%），而基线为 27--44 条（2.0--3.2 倍）。
This advantage stems from detection rate: \sys{} detects 77.2\% of violations, while prompt-filter, tool-regex, and FIDES detect only 34--40\%.
...
We additionally report three diagnostic submetrics from the same checklist results: \emph{user-query reward} for user-task checks only, \emph{implementation/test reward} for implementation, testing, configuration, and modification checks, and \emph{compliance reward} for compliance-typed checks.
% 我们另外报告来自相同清单结果的三个诊断子指标：\emph{用户查询奖励}（仅用户任务检查）、\emph{实现/测试奖励}（实现、测试、配置和修改检查）和 \emph{合规奖励}（合规类型检查）。
...
% 我们在两个条件下运行每个任务：\emph{基线}（无执行的 OpenHands）和 \emph{\sys{}}（带加载每任务策略的操作系统级 \sys{} 监视器的 OpenHands）。
...
% 此设置在两个维度上与 RQ1--RQ4 不同：智能体框架（OpenHands，而非 Codex CLI）和工作负载（工作场所安全任务，而非仓库编码任务）。
