...
\emph{AI agent intent and directives} is expressed through natural-language under project or task context, but enforcement must decide over
\emph{system actions}: concrete commands, file operations, network
connections, and subprocess effect. Runtime prompt, agent-action, and
tool-call policy
...
task context that only the agent possesses in its natural language intent.
...
task context to enforce its own actions. This creates three challenges: policies must be
concrete enough for OS enforcement while expressive enough to handle real world directives, preserve cross-event execution state,
and evolve dynamically without allowing agents or sub-agents to weaken inherited constraints.
