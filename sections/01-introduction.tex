...
harnesses can provide these instructions to the model.
Because agents comply with instructions
probabilistically~\cite{liu2024lost,jiang2024followbench,qi2025agentif}
and can violate them through planning errors, shell-script side effects,
prompt-injection drift, and opaque tool
behavior~\cite{greshake2023indirectprompt,zhan2024injecagent,debenedetti2024agentdojo},
harnesses also require runtime policy mechanisms to enforce compliance~\cite{rebedea2023nemo,wang2025agentspec,shi2025progent,
costa2025fides,debenedetti2025camel}.

% ¶3: Empirical — the problem is common; LLMs can't self-enforce
Many of these policies constrain system-level actions. Our
...
\emph{AI agent intent and directives} are expressed in natural language with project or task context, but enforcement must decide over
...
74\% of system-level directives require project or
task context that is absent from low-level OS events.

% ¶4: Insight — programmable policy engine below the tool layer
We argue that an agent-harness policy engine should be
programmable by the agent itself and take effect at OS level: it should let the agent use live project and
task context to enforce its own actions. This creates three challenges.
First, policy specification must find the right granularity: natural-language
directives carry project intent but are too ambiguous to enforce, while
raw OS events are enforceable but too low-level and brittle to express
agent-facing policy. Second, cross-event enforcement must be efficient:
policy-relevant history spans tools, subprocesses, files, and network
endpoints, but checks still run on the syscall path. Third,
programmability must not become an authority
bypass: agents need to add context-specific constraints, but neither they
nor their sub-agents should be able to weaken inherited policy.
