...
that 64\% are behavioral policies, 83\% of which involve system-level
behavior, and 81\% of projects require cross-event state tracking
(\S\ref{sec:empirical}).
A policy is \emph{per-event} if it can be checked against a single
...
\emph{context gap}: 74\% of system-level policies are not
...
their own OS-level effects. Because the agent is the only entity
...
natural-language policies; ActPlane compiles them into labeled
...
(81\% of repositories) and that 74\% of system-level policies require
...
cost, and feedback effect. We measure coverage over 607~OS-enforceable
policies from the empirical study, quantify per-event and end-to-end
overhead, and evaluate repository-grounded policy compliance on a 20-task
OctoBench subset selected for system-observable policy points against
prompt-only and tool-regex baselines.
\end{enumerate}
